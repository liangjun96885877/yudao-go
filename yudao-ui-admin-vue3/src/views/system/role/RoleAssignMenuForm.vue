...
const open = async (row: RoleApi.RoleVO) => {
  ...
  // 防提权：非超管只显示自己拥有的菜单（含其祖先以保留树形）
  const all = await MenuApi.getSimpleMenusList()
  let visible: any[] = all
  try {
    const cap = await getMyCapabilities()
    if (!cap.superAdmin) {
      const owned = new Set(cap.menuIds)
      const byId = new Map<number, any>(all.map((m: any) => [m.id, m]))
      const include = new Set<number>()
      for (const m of all as any[]) {
        if (!owned.has(m.id)) continue
        let cur: any = m
        while (cur && !include.has(cur.id)) {
          include.add(cur.id)
          cur = byId.get(cur.parentId)
        }
      }
      visible = (all as any[]).filter((m) => include.has(m.id))
    }
  } catch {}
  menuOptions.value = handleTree(visible)
  // 设置数据
  formData.id = row.id
