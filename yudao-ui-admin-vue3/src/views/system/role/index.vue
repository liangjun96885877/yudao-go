<template>
      ...
      <el-table-column :width="300" align="center" label="操作">
        <template #default="scope">
          ...
          <el-button
            ...
            title="字段权限"
            type="primary"
            @click="openFieldPermForm(scope.row)"
          >
            字段权限
          </el-button>
          <el-button link type="primary" @click="openChatter(scope.row)">
            <Icon icon="ep:chat-line-square" />动态
          </el-button>
          <el-button
            v-hasPermi="['system:role:delete']"
            link
  ...
  <!-- 表单弹窗：字段权限 -->
  <RoleFieldPermForm ref="fieldPermFormRef" @success="getList" />
  <!-- 业务动态（Chatter） -->
  <ChatterDrawer ref="chatterRef" />
</template>
...
import RoleFieldPermForm from './RoleFieldPermForm.vue'
import ChatterDrawer from '@/components/chatter/ChatterDrawer.vue'
...
/** 字段权限操作 */
const fieldPermFormRef = ref()
const openFieldPermForm = (row: RoleApi.RoleVO) => {
  fieldPermFormRef.value.open(row)
}

/** 打开角色的业务动态（Chatter） */
const chatterRef = ref()
const openChatter = (row: RoleApi.RoleVO) => {
  chatterRef.value.open('system_role', row.id, row.name)
}
