...
const open = async (row: UserApi.UserVO) => {
  ...
  // 防提权：非超管只能授出自己拥有的角色;super_admin 角色永远隐藏
  const all = await RoleApi.getSimpleRoleList()
  try {
    const cap = await getMyCapabilities()
    if (cap.superAdmin) {
      roleList.value = all
    } else {
      const mine = new Set(cap.roleIds)
      roleList.value = all.filter((r: any) => mine.has(r.id) && r.code !== 'super_admin')
    }
  } catch {
    roleList.value = all
  }
